...
namespace System.Net.Http.ProtoBuf.Internal
{
    internal class FileBufferingReadStream : Stream
    {
        ...
        {
            return DrainAsync(ArrayPool<byte>.Shared, null, cancellationToken);
        }

        public async Task DrainAsync(ArrayPool<byte> bytePool, long? limit, CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            var buffer = bytePool.Rent(MaxRentedBufferSize);
            long total = 0;
            try
            {
                ...
                while (read > 0)
                {
                    ...
                    total += read;
                    read = await ReadAsync(buffer, 0, buffer.Length, cancellationToken);
                }
            }
            finally
            {
