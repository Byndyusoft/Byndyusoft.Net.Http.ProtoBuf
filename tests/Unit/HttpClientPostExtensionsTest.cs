    ...
    public class HttpClientPostExtensionsTest
    {
        ...
        public async Task PostAsProtoBufAsync_String_WhenClientIsNull_ThrowsException()
        {
            var exception = await Assert.ThrowsAsync<ArgumentNullException>(() =>
                ((HttpClient) null).PostAsProtoBufAsync(_uri, new object(), CancellationToken.None));
        ...
        public async Task PostAsProtoBufAsync_String_WhenUriIsNull_ThrowsException()
        {
            var exception = await Assert.ThrowsAsync<InvalidOperationException>(() =>
                _client.PostAsProtoBufAsync((string) null, new object(), CancellationToken.None));
            Assert.Equal(
        ...
        public async Task PostAsProtoBufAsync_Uri_WhenUriIsNull_ThrowsException()
        {
            var exception =
                await Assert.ThrowsAsync<InvalidOperationException>(() =>
                    _client.PostAsProtoBufAsync((Uri) null, new object(), CancellationToken.None));
            Assert.Equal(
                "An invalid request URI was provided. The request URI must either be an absolute URI or BaseAddress must be set.",
                exception.Message);
        }
